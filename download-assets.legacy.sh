...
create_directories() {
    log_info "Creating Docker asset directories..."

    mkdir -p "$MAPS_DIR"
    mkdir -p "$PLUGINS_DIR"

    log_success "Docker asset directories created"
}
...
# Check Docker assets that have been downloaded
check_docker_assets() {
    log_info "Checking Docker assets..."

    # Check what assets are available
    if ls "$MAPS_DIR/"*.bsp 2>/dev/null | grep -q "aim_b0n0_d8c71"; then
        log_success "Aim map available in Docker assets: $MAPS_DIR/aim_b0n0_d8c71.bsp"
    else
        log_warning "Aim map not found in Docker assets"
    fi

    if ls "$MAPS_DIR/"*.bsp 2>/dev/null | grep -q "surf_water-run_2"; then
        log_success "Surf map available in Docker assets: $MAPS_DIR/surf_water-run_2.bsp"
    else
        log_warning "Surf map not found in Docker assets"
    fi

    log_info "Docker assets checked. Maps downloaded directly to: $MAPS_DIR/"
}
